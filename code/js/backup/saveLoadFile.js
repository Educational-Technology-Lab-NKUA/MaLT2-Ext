	  document.getElementById('files').addEventListener('change', readSingleFile, false);
		...
		function openInstance(){
			var filepath = "digitalSchoolInstance/run.mlt"
			 $.get(filepath, function(data) {
		               	try {var loadedJSON= (JSON.parse (data))
			 ...
			 if (filename.length>1){
				 ...
				 if (lang[1]=="Gr")
				 	changeLanguage();
				 filepath = "Instances/" + filename[1] +".mlt"
				 $.get(filepath, function(data) {
		               		readJSON(JSON.parse(data));
		            }, "text").fail(function() {
		                 filepath = "Instances/" + filename[1] +".txt"   //old file type
		                 $.get(filepath, function(data) {
		                           oldFilesRead(data);
		                        }, "text"). fail (function () { alert ("Oops! File not found! ")});
				 })
...
//	console.log (f.type);
  ...
  function readJSON (jObj){
		var loadedCode = jObj.editorCode;
		var loadedHiddenCode = jObj.hiddenCode;
		var loadedScene= jObj.sceneCode;
			...
			for ( i = 0; i<sceneObjects.length; i ++){
				...
				if (loadedObjects.length>0) {
					for ( i=0; i<loadedObjects.length; i++) {
						o = loadedObjects[i]
						newO = createNewObject (o.type, o.name);
						console.log (newO)
						setObjectAttributes (newO.mesh, o.position, o.rotation, o.scale,o.color)
					}
				}
...
export {openInstance, getFromOpen};
