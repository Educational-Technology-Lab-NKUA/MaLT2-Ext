//event listeners etc.
import {checkTrail, init, undo, stop, axisControl, relocateCamera, resetCanvas, changePenColor, changeBGColor, changeCharacter, exportSceneObjects, loadExternalObject, checkObject} from './init.js';
import {getEditor, play, clean_oldcode, changeFontSize} from './editor.js';
import {appearSaveBox} from './saveLoadFile.js';
import {updateColor, updatePosX, updatePosY, updatePosZ, updateRotX, updateRotY,updateRotZ,updateScaleX,updateScaleY,updateScaleZ} from './objects.js'
...
checkMobile();
  ...
  initEventListeners();
  globalThis.fr = new FileReader();
  fr.onload = function(e) {
	console.log(e.target.result);
	loadExternalObject (e)		 
}
}
...
function initEventListeners () {
  ...
  var nc = document.getElementsByClassName("CodeMirror-sizer")
   ...
   nc[0].addEventListener( 'touchmove', touchMouveEditorListener, false );
	...
	document.getElementById("posX").addEventListener("change",function() { updatePosX(this.value);}, false);
	document.getElementById("posY").addEventListener("change",function() { updatePosY(this.value);}, false);
	document.getElementById("posZ").addEventListener("change",function() { updatePosZ(this.value);}, false);
	document.getElementById("rotX").addEventListener("change",function() { updateRotX(this.value);}, false);
	document.getElementById("rotY").addEventListener("change",function() { updateRotY(this.value);}, false);
	document.getElementById("rotZ").addEventListener("change",function() { updateRotZ(this.value);}, false);
	document.getElementById("scaleX").addEventListener("change",function() { updateScaleX(this.value);}, false);
	document.getElementById("scaleY").addEventListener("change",function() { updateScaleY(this.value);}, false);
	document.getElementById("scaleZ").addEventListener("change",function() { updateScaleZ(this.value);}, false);
	document.getElementById("objColor").addEventListener("change",function() { updateColor(this.value);}, false);
