...
import {Logo,Tokenizer} from './logo.js';
import {updateOldcodeText} from './editor.js';
import {loadObjSettings} from './objects.js'
import {createSliders, removeSliders} from './sliders.js';
import Stats from 'three/addons/libs/stats.module.js';
import { STLExporter } from 'three/addons/exporters/STLExporter.js';
import {setUpCommunication, sendReady} from './states.js';
import { maltWidget } from './malt.js';
import { MTLLoader } from 'three/addons/loaders/MTLLoader.js';
	...
	globalThis.sceneObjects = [];
	globalThis.loging = true;
	globalThis.cameraView = "2dview"
	globalThis.cameraType = "null"
	globalThis.traceCode = [];
	 globalThis.defaultColor = null;
	globalThis.rotationsStarted = false;
	globalThis.cameraEventSent =false;
	globalThis.codeRange = {};
	globalThis.sceneMeshes = [];
let myTurtle, turtleGeometry, stats, controls;
globalThis.turtle = null;
globalThis.multyExecut = false;
var  zoomInEvent = false;
var zoomOutEvent = false;
var rotateEvent = false;
...
globalThis.lastDownTarget = document;
var tdaxisY = null;
var tdaxisX = null;
var dragIndex = null;
var scene = null;
...
function setup () {
  ...
  color: parameters.TURTLE_START_COLOR
	...
	init_turtle (	function (){
		setUpCommunication();
		if(onplatform){
		maltWid = new maltWidget(editor);
		maltWid.initializeLogParams();
		sendReady ();}
});
}
...
export function init (canvas_id,turtle_id,form_id,oldcode_id,textoutput_id) {
	...
	var	inner_canvas = document.getElementById("inner");
	globalThis.notes = document.getElementById ("notes");
	var	tips = document.getElementById ("tips");
		tips.style.visibility = "hidden";
	globalThis.characters = document.getElementById("charSel");
...
function init_canvas (canvas, inner_canvas) {
	...
	setControlsOptions(controls);
		...
		var material = new THREE.LineBasicMaterial({color :0xB8B8A0});
...
function setControlsOptions (controls) {
	...
	spotLight.position.set( 50, 1000, 50 );
...
function cameraEnd () {
	if(onplatform)
	maltWid.cameraAction("rotate"); //send Event Log
	globalThis.cameraView = "3Dview";
}
...
checkWindowDimensions ();
	...
	export function deleteObject(o) {
		...
		for (var i=0; i <sceneShapes.length; i++) {
			if (sceneShapes[i].mesh == o) {
				sceneShapes.splice(i,1);
			}
	
		}
		for (var i = 0; i < shapeMeshes.length; i++){
			if (shapeMeshes[i] == o)
				shapeMeshes.splice(i,1);
		}
	}
		 ...
		 newCode = fname ;
			 ...
			 code = "";
				 ...
				 //console.log (logo.usedFunctions[i]);
					 ...
					 for(var j=0; j< historyCommand[i].argsVal.length; j++) {
						var commandArg = historyCommand[i].argsVal[j];
						if (command == "make" && j==0) {
							command = command + ' "'+ commandArg.data[0]
						}
						else
...
//	controls.enabled=false;
	...
	camera.updateProjectionMatrix();
	if(onplatform)
	maltWid.cameraAction("zoomIn"); //send Event log
	}
...
function ZoomOut () {
	...
	camera.updateProjectionMatrix();
	if(onplatform)
	maltWid.cameraAction("zoomOut"); //send Event log
	}
function twoDimensions	() {
	...
	scene.remove(gridYZ);
		showGrid = false;
		if(onplatform)
		maltWid.cameraAction("switch_view"); //send Event Log
		globalThis.cameraView = "2dview";
	}
export function checkObject (clickedX, clickedY){  //checks if the user clicked on an object
			 ...
			 mouse.x = (clickedX / renderer.domElement.clientWidth ) * 2 - 1;
				...
				//  console.log (intersects);
				  if ( intersects.length != 0 ) {
					...
					for (var i =0; i<sceneShapes.length; i++) {
						if (sceneMeshes[i].id == id){
							loadObjSettings (sceneMeshes[i],clickedX, clickedY);
							break;
						}
						}
...
export function retrieveArgs (tok){
	var tokenArgs = []
...
//camera.lookAt (myTurtle.position )
	...
	// TURTLE
		...
		var  deg2rad, getPerpVec, parameters, root, scene, centroid;
		 var helper = null;
			...
			return this.position = newPosition;
			  ...
			  Turtle3D.prototype.setxy = function (x, y) {
				  ...
				  newPosition = new THREE.Vector3(x,y, this.position.z);
						...
						this.head.position.z = this.position.z;
		  this.helper.position.x = this.position.x;
		  this.helper.position.y = this.position.y;
	...
	this.helper.position.z = this.position.z;
	    ...
	    Turtle3D.prototype.getPos = function () {
		  ...
		  Turtle3D.prototype.setColor = function(r, g, b) {
			...
			var defaultColor = rgbToHex (r,g,b);
		  };
		  ...
		  ////// KANEI RETRIEVE TOUS KYLINDROUS POU THA ZWGRAFISOUN TI DIADROMI (dropings) ////
			...
			const createTransformationMatrix = function ()  {
			    ...
			    return function ( matrix, from, direction, to, width ) {
			        ...
			        matrix.compose( position, quaternion,  scale);
							//matrix.makeTranslation(0, direction.length() / 2, 0);
			...
			//CODE WITH INSTANCE MESH -- DOESNT EXPORT OBJ PROPERLY THOUGH
...
*/
    ...
    var mesh = new THREE.Mesh( turtleTrace,new THREE.MeshBasicMaterial( {color: drawCol} ) );
		...
		sceneObjects.push(newObj);
		 _results.push(mesh);
		  ...
		  export function loadExternalObject (e) {
			var contents = e.target.result;
			var loader = new OBJLoader();
			var geometry = loader.parse( contents );

			var material = new THREE.MeshBasicMaterial(({color: parameters.OBJECT_COLOR})); 
			for ( var i = 0, l = geometry.children.length; i < l; i ++ ) {
				geometry.children[ i ].material = material;
				geometry.children[ i ].receiveShadow = true;
			}
			//geometry.scale.x = 2.5;
			//geometry.scale.y = 2.5;
			//geometry.scale.z = 2.5;
			...
			scene.add(geometry);
			sceneShapes.push (geometry);
			sceneMeshes.push (geometry.children[0]);
	...
	//var texture = THREE.ImageUtils.loadTexture( 'code/turtle/Feather-Free-Texture.jpg' );
		...
		function init_turtle (_callback) {
			var turtleMaterial, ambLight;
			let object;
	 		 turtleMaterial = new THREE.MeshLambertMaterial({
	   		   color: parameters.TURTLE_START_COLO
	  		  });
			  var 	defaultColor = parameters.TURTLE_START_COLOR;
